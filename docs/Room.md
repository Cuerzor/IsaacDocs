...
???+ note "GridPathThreshold 说明"
    ...
    **3999** : 由Grimace设置。使该格子对寻路者无效。阻碍地面玩家移动。降至900后随时间以100为步长衰减(Grimace每帧重置该值)。
___
...
???+ note "说明"
    ...
    **1**: 房间被减速,可能是因为Broken Watch或I'm Drowsy药丸

    **2**: 房间被加速,可能是因为Broken Watch或I'm Excited!!!药丸
...
这给出了当前层的总恶魔房百分比。它不会像在found hud中看到的那样将其拆分为恶魔和天使百分比。它实际上是Duality的百分比。这将为非法楼层(例如Basement I)返回一个值,所以请小心。该值可以大于100%。

???- example "示例代码"
    ```lua
    -- 此代码展示了如何将room:GetDevilRoomChance转换为found hud中显示的单独的恶魔和天使百分比
    -- 此代码截至2024年1月的Repentance版本,其他版本可能有不同的值
    local game = Game()
...
返回激光束(Technology、Robo-Baby)的命中位置。通常这是直线上遇到的第一个粑便、火、岩石、TNT或墙壁。

___
### Get·Lava·Intensity () {: aria-label='Functions' }
[ ](#){: .reporplus .tooltip .badge }
#### float GetLavaIntensity ( ) {: .copyable aria-label='Functions' }
通常返回1,除非熔岩正在被Flush!或其他房间洪水效果冷却,在这种情况下它将逐渐减少到0。
...
返回Double Trouble房间中第二个boss的boss ID。否则返回0。

boss ID不等于boss的实体类型;它是entities2.xml文件中"bossID"属性里的一个单独的值。

检查此值并不足以检测Double Trouble房间,因为Double Trouble房间可能包含两个相同的boss。如果是这种情况,那么第二个boss ID的值将等于0。
...
当 `NoDecrease` 为true时,返回的收集品将不会从它们来自的池中移除。
...
返回房间当前是否受到I'm Drowsy药丸的影响。如果之前调用了 [SetSlowDown](#setslowdown) 且指定的 `Duration` 尚未过期,该函数也将返回 `true`。

请注意,如果I'm Drowsy药丸的效果是通过Broken Watch触发的,此函数将返回 `false`。要检查该场景,请使用 [GetBrokenWatchState](#getbrokenwatchstate) 函数。

如果玩家受到Stop Watch的影响,此函数也将返回 `false`。要检查该场景,请检查玩家是否拥有Stop Watch。
...
如果当前房间是XL楼层上的第二个boss房,则返回 `true`。否则返回 `false`。
...
例如,Caves中有一个相对常见的1x1房间,有四只 Boom Fly和一座从上门到下门的窄桥。在这个房间中,左右两侧的门被禁用。在这个房间中,此方法将对 `DoorSlot.UP0` 和 `DoorSlot.DOWN0` 返回true,对所有其他值返回false(无论有哪些门存在)。
...
对于天使房间是必需的,以便当天使生成时可以将清除标志设置为false。
...
有效地像使用Reroll Machine一样,补充商店库存并重新滚动物品。
...
尝试在给定的陷阱上创建一座桥。如果创建成功则返回 `true`。否则返回 `false`。
...
尝试生成一扩71f5Blue Womb的门。
除非Mom's Heart的boss房外,除非将 `Force` 设置为 `true`,否则通常什么都不会发生。
...
尝试生成一扣71f5Boss Rush的门。
除非Mom的boss房外,除非将 `Force` 设置为 `true`,否则通常什么都不会发生。
...
尝试生成一扣71f5恶魔房或天使房的门。
在非boss房内,除非将 `Force` 设置为 `true`,否则通常什么都不会发生。
...
尝试生成一扣71f5Mega Satan的门。
除非Chest/Dark Room的起始房间外,除非将 `Force` 设置为 `true`,否则通常什么都不会发生。
...
尝试生成一扣71f5包含Void传送门的房间的门。
除非Hush的boss房外,除非将 `Force` 设置为 `true`,否则通常什么都不会发生。

___
### Turn·Gold () {: aria-label='Functions' }
[ ](#){: .alldlc .tooltip .badge }
#### void TurnGold ( ) {: .copyable aria-label='Functions' }
对房间中的所有Grid Entity应用金色色调。这与游戏在击败Ultra Greedier后所做的效果相同。
...
???+ bug "Bug"
    如Repentance API Issue Tracker中所述,[作为卡牌功能的一部分调用room:Update()会强制立即使用口袋主动物品](https://github.com/Meowlala/RepentanceAPIIssueTracker/issues/211)。
